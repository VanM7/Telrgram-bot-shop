...
async  def process_address_cart_back(message: Message,state: FSMContext):
    async with state.proxy() as data:
...
@dp.message_handler(IsUser(),
                    lambda message: message.text not in [confirm_message,back_message],state=CheckoutState.confirm)
